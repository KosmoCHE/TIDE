...
class FrozenLakeRunner(TaskRunner):
    def __init__(
        ...
        for i, row in enumerate(rows):
            for j, cell in enumerate(row):
                if cell == "P":
                    p_position = (i, j)
        return_ = ""
        if g_position is not None:
            return_ += f"The target is at ({g_position[0]},{g_position[1]})."
        if len(o_positions) > 0:
            if len(o_positions) == 1:
                return_ += (
                    f"The hole is at ({o_positions[0][0]},{o_positions[0][1]})."
                )
            else:
                return_ += f"The holes are at "
                for hole in o_positions:
                    return_ += f"({hole[0]},{hole[1]}) and "
                return_ = return_[:-5]
                return_ += "."
        if p_position is not None:
            return_ += f"The player is at ({p_position[0]}, {p_position[1]})."
        return return_
    ...
    def _prepare_step_history(self, traj: TrajectoryInfo):
        ...
        observation = traj.env.render() + "\n" + self.get_state_description(traj.env.render())
